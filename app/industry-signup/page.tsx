...
import { ArrowLeft, Check, Mail, Phone, MessageCircle, Building, User } from "lucide-react"
...
export default function IndustrySignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.name.trim() || !formData.email.trim() || !formData.company.trim() || !formData.industry.trim()) return

    setIsSubmitting(true)
    try {
      const { error } = await supabase.from("industry_leads").insert([
        {
          name: formData.name,
          ...
          {!isSubmitted ? (
            <div className="bg-white/90 backdrop-blur-xl border border-white/20 rounded-3xl overflow-hidden shadow-2xl p-8">
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600 text-lg">Tell us about your industry and company</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Tell us about your automation needs</label>
                  <textarea
                    value={formData.message}
                    onChange={(e) => handleInputChange("message", e.target.value)}
                    rows={4}
                    className="w-full px-4 py-3 bg-white/60 border border-gray-200/50 rounded-xl text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/50 focus:border-[#D4AF37]/50 transition-all duration-200 backdrop-blur-sm resize-none"
                ...
                <button
                  type="submit"
                  disabled={!formData.name.trim() || !formData.email.trim() || !formData.company.trim() || !formData.industry.trim() || isSubmitting}
            ...
            <div className="bg-white/90 backdrop-blur-xl border border-white/20 rounded-3xl overflow-hidden shadow-2xl p-8 text-center">
              ...
              <p className="text-gray-600 leading-relaxed max-w-md mx-auto mb-8">
                We've received your information and will reach out to you within 24 hours to discuss how we can help automate your industry processes.
              </p>
              <div className="bg-white/40 border border-gray-200/50 rounded-xl p-6 mb-8 backdrop-blur-sm">
                ...
                <div className="space-y-2 text-gray-700 text-left">
                  <p>✓ We'll review your industry information</p>
                  <p>✓ Schedule a discovery call</p>
                  <p>✓ Analyze your automation opportunities</p>
                  <p>✓ Create a custom automation roadmap</p>
                </div>
              </div>
